            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'qa_server_credentials', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$qa_ip \"docker run --restart always --name postfix_catch_all -p 25:25 -td frouk/postfix_catch_all:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
